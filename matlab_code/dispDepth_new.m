function dispDepth_new(velEvents, depth, SHOW_VELOCITY, dt, filename, cmax)
%
...
IMAGE_FRAME = [260, 346]; %[128,128]; % image frame
...
% dd = (depth-min(depth(depth > 0)))./(max(depth(depth<1))-min(depth(depth > 0))); % normalized depth value
% depth=depth > 0.5;
% depth=depth<0.6;
% dd = (depth(depth > 0.5 & depth < 0.6)-min(depth(depth > 0.5)))./(max(depth(depth<0.6))-min(depth(depth > 0.5)))
% dd = (depth-min(depth))*1000-10;
% dd = floor(dd);
l1=length(depth)
l2=length(velEvents(:,4))
% dd = cat(2,dd0, velEvents(:,4));
dd = cat(2,depth, velEvents(1:l1,4));

% create linear colormap red to yellow
cmap = colormap(hsv);%(autumn);%
cmap = zeros(length(cmap)+2,3);
cmap(2:end-1,:) = colormap(hsv);%(autumn);%
caxis([0, cmax]);
cmap(end,:) = [1,1,1];
cmap(1,:) = [0,0,0];
...
%         d_tmp = d_tmp(d_tmp(:)==i,:);
        ...
        for l = 1:length(d_tmp(:,2)) % loop through these events and fill matrices
%             l
%             length(d_tmp(:,2))
%             length(tmp(:,4))
            x = tmp(l,1)+1;
            ...
            colormat(y,x) = floor(d*64);%t_disp; % assign color value to event
