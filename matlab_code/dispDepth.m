function dispDepth(velEvents, depth, SHOW_VELOCITY, dt, filename, cmax)
%
...
velEvents(:,4) = ceil(velEvents(:,4)./1000); % round data to ms t
velEvents(:,7) = ceil(velEvents(:,7)./1000); % duration
...
if(dt < 0)
    ...
    for i = 0:endtime % run through data in milliseconds steps
        ...
        imagemat = logical(timemat).*colormat; % logical(timemat)--remove false/0 values
%         imagemat = d_tmp.*colormat;
